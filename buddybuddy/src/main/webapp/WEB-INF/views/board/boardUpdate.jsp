...
<%-- 				<c:if test="${ !empty category}"> 
				...
				<div class="form-inline mb-2">
					...
					<input type="text" class="form-control" id="notiTitle" name="notiTitle" size="70" value="${board.notiTitle}">
				</div>

				<div class="form-inline mb-2">
					<label class="input-group-addon mr-3 insert-label">작성자</label>
					<h5 class="my-0" id="writer">${board.memberNickname}</h5>
				</div>


				<div class="form-inline mb-2">
					<label class="input-group-addon mr-3 insert-label">작성일</label>
					<h5 class="my-0" id="today">${board.notiCreateDate}</h5>
				</div>
               ...
               <c:forEach items="${board.atList}" var="at">
                  <c:choose>
                     <c:when test="${at.notifileLevel == 0 && !empty at.notifileName}">
                        <c:set var="img0" value="${contextPath}/${at.notifilePath}${at.notifileName}"/>
                     </c:when>
                     <c:when test="${at.notifileLevel == 1 && !empty at.notifileName}">
                        <c:set var="img1" value="${contextPath}/${at.notifilePath}${at.notifileName}"/>
                     </c:when>
                     <c:when test="${at.notifileLevel == 2 && !empty at.notifileName}">
                        <c:set var="img2" value="${contextPath}/${at.notifilePath}${at.notifileName}"/>
                     </c:when>
                     <c:when test="${at.notifileLevel == 3 && !empty at.notifileName}">
                        <c:set var="img3" value="${contextPath}/${at.notifilePath}${at.notifileName}"/>
                     </c:when>
                  </c:choose>
               </c:forEach>
...
<%-- 					<div class="form-inline mb-2">
 ...
 --%>
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="notiContent" name="notiContent" rows="15" style="resize: none;">${board.notiContent}</textarea>
				</div>
				...
				<div class="text-center">
					...
					<a class="btn btn-primary" href="${param.notiNo}?cp=${param.cp}">이전으로</a>
				</div>


				<input type="hidden" name="cp" value="${param.cp}">
				<input type="hidden" name="notiNo" value="${board.notiNo}">
	...
	<script>
		...
		function boardValidate() {
			if ($("#notiTitle").val().trim().length == 0) {
				alert("제목을 입력해 주세요.");
				$("#title").focus();
				return false;
			}

			if ($("#notiContent").val().trim().length == 0) {
				alert("내용을 입력해 주세요.");
				$("#content").focus();
				return false;
			}
			...
			// 유효성 검사를 모두 통과한 경우에
			// input type="hidden" 태그 중 name 속성 값이 "deleteImages"인 요소에
		...
		function LoadImg(value, num) {
			if (value.files && value.files[0]) {
				...
				reader.onload = function(e) {
					...
					//**************************************************************** 
					// 이미지가 추가된 경우 추가된 레벨이 deleteImages 배열에 있는지 검사
					// 있을 경우 해당 배열 요소를 제거
					...
					if(index != -1){ // 배열에 일치하는 레벨이 존재하는 경우
						...
						// : 배열 내부 요소 중 특정 인덱스부터 시작하여 지정된 개수만큼의 요소를 제거
		...
		$(".deleteImg").on("click", function(event) {
			...
			event.stopPropagation(); // 이벤트 버블링(이벤트가 연달아 시작되는 것)을 삭제
			
			// console.log($(this).prev().attr("src")); // img의 src 속성값 반환
			
			if( $(this).prev().attr("src") != undefined ){ // 이미지가 있을 경우에만 x버튼 동작 활성화
			
				// 3) 4개의 .deleteImg 중 몇 번째 인덱스의 x버튼이 눌러졌는지 확인
				// 왜? index == 0,1,2,3 으로 존재 == fileLevel
				const index = $(this).index(".deleteImg");
				// 클릭된 x 버튼(this)이 .deleteImg 중 몇 번째 인덱스인지 반환
				...
				$(this).prev().removeAttr("src"); // 클릭한 x버튼 이전 요소의 속성 중 src 제거
