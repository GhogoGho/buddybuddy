...
<body>
	...
	<div class="container my-5">
		...
		<h3 align="center">${pagination.boardName}</h3>
			<div class="list-wrapper">
				<table class="table table-hover table-success my-5" id="list-table">
					<thead>
						<tr>
							<th>글번호</th>
							<!-- <th>카테고리</th> -->
							...
							<c:otherwise>
								...
								<c:forEach items="${boardList}" var="board">
									<tr>
										...
										<%-- 글 제목 --%>
										<td class="boardTitle">                                                         
											<a href="${board.notiNo}?cp=${pagination.currentPage}${searchStr}">                                
												
												<%-- 썸네일 출력 --%>
												<c:choose>
													<%-- 썸네일 이미지가 없는 경우 --%>
													<c:when test="${ empty board.atList || board.atList[0].notifileLevel != 0 }">
														<img src="${contextPath}/resources/images/noimage.png">
													</c:when>
													
													<%-- 썸네일 이미지가 있는 경우 --%>
													<c:otherwise>
														<img src="${contextPath}/${board.atList[0].notifilePath}${board.atList[0].notifileName}">
													</c:otherwise>
										...
										<%-- 조회수 --%>
										<td> ${board.notiReadCount} </td>
										
										<%-- 작성일 --%>
										<td> 
											<fmt:formatDate var="createDate" value="${board.notiCreateDate}"  pattern="yyyy-MM-dd"/>                          
											...
											<c:choose>
												...
												<%-- 글 작성일이 오늘일 경우 --%>
												<c:otherwise>
													<fmt:formatDate value="${board.notiCreateDate}"  pattern="HH:mm"/>                          
												</c:otherwise>
											</c:choose>
										</td>
									</tr>
